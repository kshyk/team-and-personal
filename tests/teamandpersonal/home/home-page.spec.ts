...
import HomePage, {
  OfferPuzzle
} from '../../../pageobjects/teamandpersonal/home/home-page';
...
test.describe('HomePage', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await new Header(page).openHomePageBy('Menu');
    await page.waitForURL('https://teamandpersonal.pl/');
  });

  test.describe('Tuckman Chart', async () => {
    test('Welcome text and title', async () => {
      await homePage.checkWelcomeText(
        'Witamy na stronie Team&Personal, firmy specjalizującej się w budowaniu zespołów. ' +
  ...
  test.describe('Offer Puzzles', () => {
    test('Integration Trips', async ({ page }) => {
      await checkOfferPuzzle(
        ...
        'Wyjazdy integracyjne',
        'Wspólny wyjazd czy impreza integracyjna może być pierwszym krokiem w „formowaniu” zespołu. To czas na poznanie siebie i swoich możliwości. W naszej ofercie znajdziecie krótkie programy indoorowe jak i rozbudowane scenariusze na lądzie, wodzie i w powietrzu. Wśród nich m.in. autorskie gry miejskie oraz gry terenowe.',
        '/wyjazdy-integracyjne/'
      );
    });
    ...
    test('Events', async ({ page }) => {
      await checkOfferPuzzle(
        ...
        'Eventy',
        'Eventy',
        'Na każdym etapie życia zespołu jest miejsce na Event, wieczorną zabawę, czy konferencję tematyczną służącą podsumowaniu lub inauguracji. W naszej ofercie znajdziecie Państwo propozycje od kameralnych programów artystycznych, wieczorów tematycznych po kompleksową obsługę techniczną i artystyczną.',
        '/eventy/'
      );
    });

    const checkOfferPuzzle = async (
      page: Page,
  ...
  test.describe('Realisations', () => {
    test('Validate realisations with tags', async () => {
      ...
      await homePage.checkTagsExistence([
        'TEAM BUILDING',
        'EVENTY',
        'COACHING',
        'IMPREZY FIRMOWE'

...
export default class HomePage {
  constructor(private readonly page: Page,
              ...
              private readonly referenceNavigator = page.locator('ol.flex-control-paging'),
  ...
  checkOfferPuzzleTitle = async (offerPuzzle: OfferPuzzle, expectedTitle: string) => {
    switch (offerPuzzle) {
      case 'Wyjazdy integracyjne':
        expect(await this.offers.integrationTrips.innerText()).toEqual(expectedTitle);
        break;
      case 'Szkolenia':
        expect(await this.offers.trainings.innerText()).toEqual(expectedTitle);
        break;
      case 'Team building':
        expect(await this.offers.teamBuilding.innerText()).toEqual(expectedTitle);
        break;
      case 'Eventy':
  ...
  checkOfferPuzzleDescription = async (offerPuzzle: OfferPuzzle, expectedDescription: string) => {
    switch (offerPuzzle) {
      case 'Wyjazdy integracyjne':
        await expect(this.offers.integrationTrips.locator('span')).toHaveText(expectedDescription);
        break;
      case 'Szkolenia':
        await expect(this.offers.trainings.locator('span')).toHaveText(expectedDescription);
        break;
      case 'Team building':
        await expect(this.offers.teamBuilding.locator('span')).toHaveText(expectedDescription);
        break;
      case 'Eventy':
        await expect(this.offers.events.locator('span')).toHaveText(expectedDescription);
        break;
    }
  }
  ...
  checkReferenceDescription = async (index: number, expectedDescription: string) => {
    await expect(this.references.description.nth(index)).toHaveText(expectedDescription);
  }

  checkReferenceCompanyName = async (index: number, expectedCompanyName: string) => {
    await expect(this.references.companyName.nth(index)).toHaveText(expectedCompanyName);
  }
  ...
  checkNumberOfReferences = async (expectedNumber: number) => {
    await expect(this.referenceNavigator.locator('li')).toHaveCount(expectedNumber);
  }
